...
public:
  ...
  // Sensor plugins can register their owning model here.  The map system
  // checks the entity ancestry of every visual and excludes descendants of
  // registered models.  This avoids hard-coding Gazebo's generated model
  // suffixes (for example `_0`) in a world file.
  void AddIgnoredModelEntity(gz::sim::Entity entity)
  {
    std::lock_guard<std::mutex> lock(mutex_);
    ignored_model_entities_.insert(entity);
  }

  void RemoveIgnoredModelEntity(gz::sim::Entity entity)
  {
    std::lock_guard<std::mutex> lock(mutex_);
    ignored_model_entities_.erase(entity);
  }

  bool IsIgnoredModelEntity(gz::sim::Entity entity) const
  {
    std::lock_guard<std::mutex> lock(mutex_);
    return ignored_model_entities_.count(entity) != 0;
  }
...
private:
  ...
  std::unordered_set<gz::sim::Entity> ignored_model_entities_;
};
