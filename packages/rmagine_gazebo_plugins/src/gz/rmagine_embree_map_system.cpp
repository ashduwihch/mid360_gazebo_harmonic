  ...
  const gz::sim::EntityComponentManager &_ecm)
{
  ...
  tracked_visuals_[entity] = TrackedVisual{
    embree_geom, geom_id, pose, GeometryKey(geom), IsMovableVisual(entity, _ecm)};
  return true;
}
  ...
  const gz::sim::EntityComponentManager &_ecm)
{
  ...
  for(const auto entity : entities)
  {
    ...
    if(embree_geom)
    {
      const unsigned int geom_id = scene_->add(embree_geom);
      tracked_visuals_[entity] = TrackedVisual{
        embree_geom, geom_id, pose, new_key, IsMovableVisual(entity, _ecm)};
    }
    changed = true;
  }
  ...
  const gz::sim::EntityComponentManager &_ecm)
{
  ...
  for(auto &kv : tracked_visuals_)
  {
    ...
    // Static factory buildings dominate large worlds.  Their poses cannot
    // change, so walking their full parent chains every map update only burns
    // CPU and gives no new information.
    if(!tracked.movable)
    {
      continue;
    }
  ...
  const gz::sim::EntityComponentManager &_ecm)
{
  ...
  if(!map_built_)
  {
    ...
    // Match Gazebo Classic's ray sensor semantics: build the ray tracing
    // scene from physical collision geometry, not render-only visuals.
    // Collision meshes are normally much simpler and avoid loading textures
    // and decorative high-poly geometry into Embree.
    ...
    if(debug_)
    {
      std::cerr << "[RmagineEmbreeMapSystem] Embree map bootstrapped with "
                << tracked_visuals_.size() << " supported collisions." << std::endl;
    }
    return;
  }
  ...
  if(topology_dirty || pose_dirty)
  {
    ...
    if(debug_)
    {
      std::cerr << "[RmagineEmbreeMapSystem] Synced scene (topology_dirty="
                ...
                << "), tracking " << tracked_visuals_.size() << " collisions." << std::endl;
    }
  }
}
  ...
  const gz::sim::EntityComponentManager &_ecm) const
{
  ...
  {
    if(MapRegistry::Instance().IsIgnoredModelEntity(current))
    {
      return true;
    }
...
bool RmagineEmbreeMapSystem::IsMovableVisual(
  gz::sim::Entity entity,
  const gz::sim::EntityComponentManager &_ecm) const
{
  auto current = entity;
  while(current != gz::sim::kNullEntity)
  {
    if(auto staticComp = _ecm.Component<gz::sim::components::Static>(current))
    {
      if(staticComp->Data())
      {
        return false;
